...
def main():
  ...
  for epoch in tqdm(range(start_epoch, args.epochs), desc = "Iterating over epochs", total = args.epochs - start_epoch):
    ...
    # training
    train_acc, train_obj = train_higher(train_queue, valid_queue, model, architect, criterion, optimizer, lr, steps_per_epoch=args.steps_per_epoch)
...
def train_higher(train_queue, valid_queue, network, architect, criterion, w_optimizer, a_optimizer, inner_steps=100, steps_per_epoch=None):
    import higher
